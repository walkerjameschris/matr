...
Let our training data be denoted as $X$, the weights for hidden layer one as $H_1$, the weights for hidden layer two as $H_2$, the weight for the final layer as $W$. We also need to denote function $f()$ which corresponds to `activation()` from above.
